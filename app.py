...
def dashboard():
    ...
    try:
        ...
        # Today: detections from today (most recent per image)
        today_subquery = db.session.query(
            Detection.image_path,
            db.func.max(Detection.timestamp).label('max_timestamp')
        ).filter(
            Detection.user_id == current_user.id,
            Detection.timestamp >= today_start,
            Detection.timestamp < tomorrow_start
        ).group_by(Detection.image_path).subquery()
        today_detections = db.session.query(Detection).join(
            today_subquery,
            (Detection.image_path == today_subquery.c.image_path) & (Detection.timestamp == today_subquery.c.max_timestamp)
        ).count()

        # Week: detections from this week (most recent per image)
        week_subquery = db.session.query(
            Detection.image_path,
            db.func.max(Detection.timestamp).label('max_timestamp')
        ).filter(
            Detection.user_id == current_user.id,
        ...
        week_detections = db.session.query(Detection).join(
            week_subquery,
            (Detection.image_path == week_subquery.c.image_path) & (Detection.timestamp == week_subquery.c.max_timestamp)
        ).count()

        # Month: detections from this month (most recent per image)
        month_subquery = db.session.query(
            Detection.image_path,
            db.func.max(Detection.timestamp).label('max_timestamp')
        ).filter(
            Detection.user_id == current_user.id,
        ...
        month_detections = db.session.query(Detection).join(
            month_subquery,
            (Detection.image_path == month_subquery.c.image_path) & (Detection.timestamp == month_subquery.c.max_timestamp)
        ).count()

        # Total: all time (most recent per image)
        ...
        total_detections = db.session.query(Detection).join(
            subquery,
            (Detection.image_path == subquery.c.image_path) & (Detection.timestamp == subquery.c.max_timestamp)
        ).count()
        
        # Ripe/Unripe totals (all time)
        ripe_total = Detection.query.filter(
            Detection.user_id == current_user.id,
            Detection.ripeness == 'ripe'
        ).count()
        unripe_total = Detection.query.filter(
            Detection.user_id == current_user.id,
            Detection.ripeness == 'unripe'
        ).count()
        
        # Mangosteen-specific stats
        mangosteen_total = Detection.query.filter(
            Detection.user_id == current_user.id,
            Detection.fruit_type.ilike('mangosteen')
        ).count()
        mangosteen_ripe = Detection.query.filter(
            Detection.user_id == current_user.id,
            Detection.fruit_type.ilike('mangosteen'),
            Detection.ripeness == 'ripe'
        ).count()
        mangosteen_unripe = Detection.query.filter(
            Detection.user_id == current_user.id,
            Detection.fruit_type.ilike('mangosteen'),
            Detection.ripeness == 'unripe'
        ).count()
        ...
        # Get daily statistics for the past 7 days
        days_ago = [today_start - timedelta(days=i) for i in range(6, -1, -1)]
        daily_labels = [(today_start - timedelta(days=i)).strftime('%a %d') for i in range(6, -1, -1)]
        daily_counts = []
        for day in days_ago:
        ...
        # Ripe/Unripe for today
        ripe_today = Detection.query.filter(
            Detection.user_id == current_user.id,
            Detection.ripeness == 'ripe',
            Detection.timestamp >= today_start,
            Detection.timestamp < tomorrow_start
        ).count()
        unripe_today = Detection.query.filter(
            Detection.user_id == current_user.id,
            Detection.ripeness == 'unripe',
            Detection.timestamp >= today_start,
            Detection.timestamp < tomorrow_start
        ).count()
        # Ripe/Unripe for week
        ripe_week = Detection.query.filter(
            Detection.user_id == current_user.id,
            Detection.ripeness == 'ripe',
            Detection.timestamp >= week_start,
            Detection.timestamp < tomorrow_start
        ).count()
        unripe_week = Detection.query.filter(
            Detection.user_id == current_user.id,
            Detection.ripeness == 'unripe',
            Detection.timestamp >= week_start,
            Detection.timestamp < tomorrow_start
        ).count()
        # Ripe/Unripe for month
        ripe_month = Detection.query.filter(
            Detection.user_id == current_user.id,
            Detection.ripeness == 'ripe',
            Detection.timestamp >= month_start,
            Detection.timestamp < tomorrow_start
        ).count()
        unripe_month = Detection.query.filter(
            Detection.user_id == current_user.id,
            Detection.ripeness == 'unripe',
            Detection.timestamp >= month_start,
            Detection.timestamp < tomorrow_start
        ...
        return render_template('dashboard.html',
                             ...
                             ripe_total=ripe_total,
                             unripe_total=unripe_total,
                             common_objects=common_objects,
                             ripeness_stats=ripeness_stats,
                             recent_detections=recent_detections,
                             mangosteen_total=mangosteen_total,
                             mangosteen_ripe=mangosteen_ripe,
                             mangosteen_unripe=mangosteen_unripe,
                             daily_labels=daily_labels,
                             daily_counts=daily_counts,
                             ripe_today=ripe_today,
                             unripe_today=unripe_today,
                             ripe_week=ripe_week,
                             unripe_week=unripe_week,
                             ripe_month=ripe_month,
                             unripe_month=unripe_month)
    except Exception as e:
        import traceback
...
def history():
    ...
    try:
        ...
        if date_from:
            ...
            query = query.filter(Detection.timestamp >= date_from_dt)
        if date_to:
            # Parse as start of the next day, so we include the whole 'date_to' day
            date_to_dt = datetime.strptime(date_to, "%Y-%m-%d") + timedelta(days=1)
            query = query.filter(Detection.timestamp < date_to_dt)
        # Remove duplicates: get only the most recent detection per image_path (for display)
        ...
        # Count filtered ripe/unripe in ALL detections matching the filter (not deduplicated)
        all_filtered = query.all()
        filtered_ripe_count = sum(1 for d in all_filtered if d.ripeness == 'ripe')
        filtered_unripe_count = sum(1 for d in all_filtered if d.ripeness == 'unripe')
        # AJAX partial rendering
        if request.headers.get('X-Requested-With') == 'XMLHttpRequest':
            return render_template('_history_container.html', detections=detections, ripe_unripe_counts=ripe_unripe_counts, filtered_ripe_count=filtered_ripe_count, filtered_unripe_count=filtered_unripe_count, request=request)
        return render_template('history.html', detections=detections, ripe_unripe_counts=ripe_unripe_counts, filtered_ripe_count=filtered_ripe_count, filtered_unripe_count=filtered_unripe_count)
    except Exception as e:
